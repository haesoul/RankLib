...
import { MediaPaths } from "./mediaPaths"; // <-- Укажи правильный путь к твоему файлу MediaPaths

// Теперь проверяем, что файл вообще лежит в нашей корневой директории media/
export function isDownloadedTempFile(path?: string): boolean {
...
// Строго задаем цель: либо качаем обложку для класса, либо для объекта
...
export async function downloadImageToLocalStorage(
  ...
  // 1. Формируем путь СТРОГО через MediaPaths
  let targetDir = "";
  ...
  // 2. Создаем директорию, если её еще нет (заменяет старый костыль ensureImagesDir)
  ...
  // 3. Формируем имя временного файла (качаем прямо в нужную папку)
  const fileId = `cover`;
